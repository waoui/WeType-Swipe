...
replace(
    ...
    'TextView version = text("v1.10.0 测试版 · 原生剪贴板与快捷发送", 13, COLOR_SECONDARY);',
)
replace(
    activity,
...
replace(hook, 'logInfo("v1.9.4 entered target package");', 'logInfo("v1.10.0-test1 entered target package");')
replace(
    hook,
...
helpers = '''
    ...
    private View findMatchingView(View view, String[] tokens) {
        ...
        if (view instanceof ViewGroup) {
            ...
            for (int index = 0; index < group.getChildCount(); index++) {
                View result = findMatchingView(group.getChildAt(index), tokens);
    ...
    private static String describeView(View view) {
        ...
        if (id != View.NO_ID) {
            try {
                value.append(view.getResources().getResourceEntryName(id));
            } catch (Throwable ignored) {
                value.append(id);
            }
        }
...
'''
replace(hook, '    private static boolean isParagraphAction(int action) {', helpers + '    private static boolean isParagraphAction(int action) {')
...
print("v1.10.0-test1 patch applied")
