...
paint_pattern = re.compile(
    r"    private void prepareKeyboardLabelPaint\(View keyboard\) \{.*?\n    \}\n\n"
    r"(?=    private void drawQwertyFunctionLabels\(View keyboard, Canvas canvas, Config config\) \{)",
    re.S)
...
'''
text, count = paint_pattern.subn(paint_replacement, text, count=1)
if count != 1:
    raise RuntimeError(f"paint method: expected 1 match, found {count}")

# Replace fixed 3x4 T9 coordinates with cached hit-test geometry.
t9_pattern = re.compile(
    r"    private void drawT9FunctionLabels\(View keyboard, Canvas canvas, Config config\) \{.*?\n    \}\n\n"
...
'''
text, count = t9_pattern.subn(t9_replacement, text, count=1)
if count != 1:
    raise RuntimeError(f"T9 draw method: expected 1 match, found {count}")

# Limit diagnostic method discovery to the self-draw pipeline instead of all View methods.
diag_pattern = re.compile(
    r"    private static boolean diagnosticMethodCandidate\(Method method\) \{.*?\n    \}\n\n"
    r"(?=    private static boolean diagnosticTencentType\(Class<\?> type\) \{)",
    re.S)
...
'''
text, count = diag_pattern.subn(diag_replacement, text, count=1)
if count != 1:
    raise RuntimeError(f"diagnostic candidate method: expected 1 match, found {count}")

# Lower per-layout noise now that the diagnostic is targeted.
...
print("v1.11.0-diag2 theme-aware and T9 geometry diagnostic patch applied")
