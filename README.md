...
- 新增“剪贴板”动作，直接打开微信输入法原生剪贴板历史。
- 新增“快捷发送”动作，直接打开原生快捷发送／常用语面板。
- 两项功能在工具栏隐藏对应按钮时仍可使用。
- 完善横屏与横屏悬浮键盘适配。
- 缓存工具栏命令载体与反射成员，减少普通输入过程中的扫描。
- 优化键位识别热路径和缓存失效恢复。
- 减少正常输入与成功触发时的日志写入。

完整变更请查看 [v1.10.0 Release](https://github.com/waoui/WeType-Swipe/releases/tag/v1.10.0)。
